...
execute at @a[team=Runners] if entity @a[team=Hunters,distance=..50] as @s run title @s[team=Runners] actionbar {"text": "There are hunters near you!", "color": "yellow"}
execute at @a[team=Hunters] if entity @a[team=Runners,distance=..50] as @s run title @s[team=Hunters] actionbar {"text": "You are near one or more runners!", "color": "yellow"}